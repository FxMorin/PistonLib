...
public class SpeedPistonBlockEntity extends BasicPistonBlockEntity {
    ...
    public SpeedPistonBlockEntity(float speed, BlockPos pos, BlockState state, BlockState pushedBlock,
    ...
    public SpeedPistonBlockEntity(float speed, BlockPos pos, BlockState state, BlockState pushedBlock, Direction dir,
                                  boolean extending, boolean source, BasicPistonExtensionBlock extensionBlock) {
        super(pos, state, pushedBlock, dir, extending, source, extensionBlock);
        ((BlockEntityAccessor)this).setType(ModBlockEntities.SPEED_PISTON_BLOCK_ENTITY);
        this.speed = speed;
        this.pistonOffset = 0.01 * speed;
    }
    ...
    public static void push(SpeedPistonBlockEntity be, BlockPos pos, Entity entity, Direction dir, double amount) {
        Box box = entity.getBoundingBox();
        Box box2 = VoxelShapes.fullCube().getBoundingBox().offset(pos);
        if (box.intersects(box2)) {
            Direction direction2 = dir.getOpposite();
            ...
            if (Math.abs(d - e) < be.pistonOffset) {
                ...
                moveEntity(dir, entity, d, direction2);
            }
        }
    }
